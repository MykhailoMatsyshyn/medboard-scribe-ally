...
| [`rag-full-text-search`](./rag-full-text-search/SKILL.md) | Document-grounded retrieval with Postgres full-text search (generated `tsvector` + `ts_rank`) — no embeddings, runs entirely in Postgres. |
...
openrouter-forced-tool-calling → rag-full-text-search**.
